...
bands=ggplot(cbind(data,pred),aes(x=density, y=log(gain)))+
  geom_point() +
  geom_line(aes(y=lwr), color = "red", linetype = "dashed")+
  geom_line(aes(y=upr), color = "red", linetype = "dashed")+
  geom_smooth(method=lm, se=TRUE)
bands+ggtitle('Linear Regression Line and its Confidence and Prediction Bands')

x=data.frame(rep(0,9),rep(0,9),rep(0,9),rep(0,9),rep(0,9))
for (i in 1:9){
  x[i,1]=density[10*i]
  x[i,2]=mean(gain[(10*i-9):(10*i)])
  x[i,3:5]=exp(pred[10*i,1:3])
}
...
bands2=ggplot(y,aes(x=log(y[,2]), y=y[,1]))+
  geom_point() +
  geom_line(aes(y=y[,5]), color = "red", linetype = "dashed")+
  geom_line(aes(y=y[,4]), color = "red", linetype = "dashed")+
  geom_smooth(method=lm, se=TRUE)
bands2+xlab('log(gain)')+ylab('density')+
  ggtitle('Inverse Linear Regression Line and its Confidence and Prediction Bands')

#inverse prediction of any gain
inverse.predict(fit1,log(38.6))
inverse.predict(fit1,log(426.7))
